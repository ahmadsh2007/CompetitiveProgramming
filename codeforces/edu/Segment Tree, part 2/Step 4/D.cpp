...
struct Node {
    int sum; // sum of 1-based indices in segment
    int val; // sum of a[i]
    int mul; // sum of i * a[i]
    Node () : sum(0), val(0), mul(0) {}
    Node (int sum, int val, int mul) : sum(sum), val(val), mul(mul) {}
};
...
void solve() {
    ...
    auto op = [](const Node & a, const Node & b) -> Node {
        return Node(a.sum + b.sum, a.val + b.val, a.mul + b.mul);
    };
    auto map = [](const int & tag, const Node & node, const int & len) -> Node {
        return Node(node.sum, node.val + tag * len, node.mul + tag * node.sum);
    };
    auto comp = [](const int & newTag, const int & oldTag) {
        return newTag + oldTag;
    };

    Node defVal(0, 0, 0);
    LazySegTree<Node, int, decltype(op), decltype(map), decltype(comp)> lst(a, defVal, op, map, comp);
    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            int l, r; cin >> l >> r;
            Node res = lst.query(l, r + 1);
            cout << res.mul - res.val * (l - 1) << endl;
        }
    }
}
