...
void solve() {
    int n, m; cin >> n >> m;
    auto op = [](const int & a, const int & b) {
        return a + b;
    };
    auto map = [](const Tag & tag, const int & val, const int & len) {
        if (tag.add) return val + tag.upd * len;
        return tag.upd * len;
    };
    auto comp = [](const Tag & newTag, const Tag & oldTag) -> Tag {
        if (!newTag.add) return newTag;
        else return {newTag.add and oldTag.add, newTag.upd + oldTag.upd};
    };

    LazySegTree<int, Tag, decltype(op), decltype(map), decltype(comp)> lst(n, 0, op, map, comp);

    while (m--) {
        int op; cin >> op;
        if (op == 1) {
            int l, r, v; cin >> l >> r >> v;
            lst.update(l, r, {false, v});
        }
        else if (op == 2) {
            int l, r, v; cin >> l >> r >> v;
            lst.update(l, r, {true, v});
        }
        else {
            int l, r; cin >> l >> r;
