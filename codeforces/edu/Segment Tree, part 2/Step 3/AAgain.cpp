...
void solve() {
    ...
    auto op = [](Node a, Node b) -> Node {
        return {
            a.sum + b.sum,
            max({a.maxSum, b.maxSum, a.suffix + b.prefix}),
            max({a.sum + b.prefix, a.prefix}),
            max({a.suffix + b.sum, b.suffix})
        };
    };
    auto map = [](int tag, Node val, int len) -> Node {
        return {
            tag * len,
            max(0ll, tag * len),
            max(0ll, tag * len),
    ...
    auto comp = [](int newTag, int oldTag) -> int {
        return newTag;
    };

    LazySegTree<Node, int, decltype(op), decltype(map), decltype(comp)> lst(n, {}, op, map, comp);
    while (m--) {
        int l, r, v; cin >> l >> r >> v;
        lst.update(l, r, v);
        cout << lst.query(0, n).maxSum << endl;
    }
}
