...
private:
    ...
    T findJ(int l, int v, int x, int lx, int rx) {
        if (tree[x] < v or rx <= l) return -1;
        if (rx - lx == 1) return lx;

        push(x, lx, rx);

        int mid = lx + (rx - lx) / 2;
        int res = findJ(l, v, 2 * x + 1, lx, mid);
        if (~res) return res;
        return findJ(l, v, 2 * x + 2, mid, rx);
    }
public:
...
void solve() {
    ...
    auto map = [](int tag, int val, int len) {
        return tag + val;
    };
    auto comp = [](int newTag, int oldTag) {
        return newTag + oldTag;
    };
    LazySegTree<int, int, decltype(op), decltype(map), decltype(comp)> lst(n, 0, op, map, comp);
