...
int getMEX(const vector<int>& a, int L, int R) {
    vector<bool> seen(a.size() + 2);
    for (int i = L; i <= R; i++) {
        if (a[i] <= a.size()) seen[a[i]] = true;
    }
...
void solve() {
    ...
    for (int i = 0; i < n - 1; i++) if (getMEX(a, 0, i) == getMEX(a, i + 1, n - 1)) { cout << "NO" << endl; return; }
    cout << "YES" << endl;
}
