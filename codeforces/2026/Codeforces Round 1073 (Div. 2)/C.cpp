...
void solve() {
    ...
    if (is_sorted(s.begin(), s.end())) { cout << "Bob" << endl; return; }
    
    int countZeros = 0;
    for (char c : s) if (c == '0') countZeros++;

    vi(idx);
    for (int i = 0; i < n; i++) {
        if (i < countZeros && s[i] == '1') idx.push_back(i + 1);
        else if (i >= countZeros && s[i] == '0') idx.push_back(i + 1);
    }
