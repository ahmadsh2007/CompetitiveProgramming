...
void solve() {
    int n; cin >> n;
    vector<Interval> ans(n);
    for (int i = 0; i < n; ++i) {
        cin >> ans[i].l >> ans[i].r >> ans[i].u >> ans[i].v;
    }
    ...
    for (int m = n; m >= 1; --m) {
        ...
        for (int i = 0; i < n; ++i) {
            int j = size + 1;
            if (j <= m and (j < ans[i].l or j > ans[i].r) and
                (m - j + 1 < ans[i].u or m - j + 1 > ans[i].v)) size++;
        }
        if (size == m) return void(cout << size << endl);
    }
