...
void solve() {
    ...
    vi prefix(n + 10), suffix(n + 10);;
    for (int i = 1; i <= n; ++i) prefix[i] = prefix[i - 1] + abs(a[i]);
    for (int i = n; i >= 1; --i) suffix[i] = suffix[i + 1] + a[i];

    vector<int> positive;
    for (int i = 1; i <= n; ++i) if (a[i] > 0) positive.push_back(i);

    int lastEdit = -1;
    int sum = suffix[1];

    for (auto p : positive) {
        int cur = prefix[p - 1] - a[p] + suffix[p + 1];
        if (cur > sum) {
            sum = cur;
            lastEdit = p;
        }
    ...
    vector<int> edited(n + 1);
    edited[lastEdit] = 1;
    for (int i = 1; i < lastEdit; ++i) if (a[i] < 0) edited[i] = 1;

    vector<int> positives, negatives;
    for (int i = 1; i < lastEdit; ++i) {
        if (edited[i] != edited[i + 1]) {
            if (a[i] > 0) positives.push_back(i);
            else negatives.push_back(i);
        }
    }

    vi operations;
    for (int i = 0; i < (int) positives.size(); ++i) operations.push_back(positives[i]);
    for (int i = (int) negatives.size() - 1; i >= 0; --i) operations.push_back(negatives[i]);

    cout << (int) operations.size() + 1 << endl;
    cout << operations << lastEdit << endl;
}
