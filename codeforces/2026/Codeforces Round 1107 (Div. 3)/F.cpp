...
void solve() {
    ...
    if (inversions % 2) return void(cout << "Alice" << endl);
    if (firstOne == -1 || lastZero == -1 || firstOne > lastZero) return void(cout << "Bob\n");

    bool evenBlocks = 1;
    int len = 0;
    for (int i = firstOne; i <= lastZero; ++i) {
        len++;
        if (i == lastZero || s[i] != s[i + 1]) {
            if (len % 2) evenBlocks = 0;
            len = 0;
        }
    }

    if (evenBlocks) cout << "Bob" << endl;
