...
void solve() {
    ...
    vector<vector<int>> doms(n + 1);
    vector<pair<int, int>> dominos(n + 1);

    for (int i = 1; i <= n; ++i) {
        cin >> dominos[i].first >> dominos[i].second;

        doms[dominos[i].first].push_back(i);
        doms[dominos[i].second].push_back(i);
    }

    vector<vector<int>> adj(n + 1);
    for (int i = 1; i <= n; ++i) {
        if ((int) doms[i].size() > 2) return void(cout << "NO\n");

        if ((int) doms[i].size() == 2) {
            adj[doms[i][0]].push_back(doms[i][1]);
            adj[doms[i][1]].push_back(doms[i][0]);
        }
    }
