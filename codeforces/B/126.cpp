...
void solve() {
    ...
    int n = (int) s.size();
    PolyHash h(s);
    if (n < 3) return void(cout << "Just a legend\n");

    vector<int> valid;
    for (int len = 1; len < n; ++len) {
        if (h.get_hash(0, len - 1) == h.get_hash(n - len, n - 1)) {
            valid.push_back(len);
        }
    }

    if (valid.empty()) return void(cout << "Just a legend\n");

    auto check = [&](int len) -> bool {
        auto prefixHash = h.get_hash(0, len - 1);
        int mx = n - 1 - len;
        for (int i = 1; i <= mx; ++i) {
            if (h.get_hash(i, i + len - 1) == prefixHash) return true;
        }
        return false;
    };
