...
void solve() {
    ...
    auto ans = [&](auto ans, int c, int k, int d) -> int {
        if (k == 1) return 1;
        if (dp[c][k][d] != -1) return dp[c][k][d];
        int anss = 2;
        if (d == 1) {
            if (c < n) anss += ans(ans, c + 1, k, d) - 1;
            anss %= MOD;
            if (c > 1) anss += ans(ans, c - 1, k - 1, 1 - d) - 1;
            anss %= MOD;
            dp[c][k][d] = anss;
        }
        else {
            if (c > 1) anss += ans(ans, c - 1, k, d) - 1;
            anss %= MOD;
            if (c < n) anss += ans(ans, c + 1, k - 1, 1 - d) - 1;
            anss %= MOD;
            dp[c][k][d] = anss;
        }
        return anss;
