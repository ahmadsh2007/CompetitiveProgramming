...
void solve() {
    ...
    vector<int> prefix(n);
    for (int i = 0; i < n; ++i) {
        if (b[s[i] - 'a'] == '0') prefix[i] = 1;
    }
    for (int i = 1; i < n; ++i) prefix[i] += prefix[i - 1];

    gp_hash_table<u128, null_type, custom_hash> ans;
    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {
            if (prefix[j] - (i ? prefix[i - 1] : 0) > k) break;
            auto check = h.get_hash(i, j);
            u128 hashed = ((u128) check.first << 64 | (u128) check.second);
            ans.insert(hashed);
        }
    }
    cout << ans.size() << endl;
